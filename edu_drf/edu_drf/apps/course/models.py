...
class Course(BaseModel):
    ...
    def show_lesson_list(self):
        print('60显示的课时',self)
        # 查询当前课程下的前四个课时
        show_lesson_list = CourseLesson.objects.filter(is_show=True, is_delete=False, course_id=self.id).all()[:4]
        # 定义一个新列表，存放课时id、名字以及是否免费 信息
        data_list= []
        for lesson in show_lesson_list:
            data_list.append({
                'id':lesson.id,
                'name':lesson.name,
    ...
    @property
    def level_zh(self):
        for i in self.level_choices:
            if i[0] == self.level:
                return i[1]

    @property
    def brief_html(self):
        # 获取单个课程详情介绍信息 要显示的html
        # print(84, self.brief)
        # 将上传的图片路径替换原有的本地路径
        brief_html = self.brief.replace('src="/media', 'src="%s/media' % IMG_URL)
        # print(86, brief_html)
